...
int main(){
    ...
    std::cout << "Perimetrul triunghiului: " << t1.perimeter() << "\n";
    
    Circle* c1 = Circle::createCircle( Point(0, 0), 10);
    std::cout << "Circumferinta cercului : " << c1->perimeter() << "\n";
    
    try{
        Circle* c2 = Circle::createCircle( Point(0, 0), 5);
        std::cout << "Circumferinta cercului : " << c2->perimeter() << "\n";
    }
    catch(int i){
        std::cout << "Doar un cerc e permis \n";
    }
    
    std::cout << "Slope triunghi: " << slope(t1) << "\n";
    
    Square s1 = Square( Point(11, 11), 4 );
    std::cout << "Perimetrul patratului : " << s1.perimeter() << "\n";
    
    std::cout << "Cerc si triunghi se intersecteaza  " << intersect(t1, *c1) << "\n";
    std::cout << "Cerc si patrat se intersecteaza " << intersect(s1, *c1) << "\n";
    return 0;
}
